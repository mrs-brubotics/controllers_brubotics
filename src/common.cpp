// copied from https://github.com/ctu-mrs/mrs_uav_controllers/blob/master/src/common.cpp
#include <common.h>
...
std::optional<sanitizeDesiredForceandTilt> sanitizeDesiredForce(const Eigen::Vector3d& input, const double& tilt_safety_limit, const double& tilt_saturation,
  ...
  if (tilt_saturation > 1e-3 && std::abs(theta) > tilt_saturation) {
    ROS_WARN_THROTTLE(1.0, "[%s]: tilt is being saturated, desired: %.2f deg, saturated %.2f deg", node_name.c_str(), (theta / M_PI) * 180.0,
                      ...
                      // TODO: change 1.0 rate to ROS_INFO_THROTTLE_PERIOD by loading new variables
    theta = tilt_saturation;
  }

  // reconstruct the force vector back out of the spherical coordinates
  Eigen::Vector3d output_force(sin(theta) * cos(phi), sin(theta) * sin(phi), cos(theta));
  sanitizeDesiredForceandTilt output;
  output.force = output_force;
  output.tilt = theta;
  return {output};
}
...
std::optional<mrs_msgs::HwApiAttitudeRateCmd> attitudeController(const mrs_msgs::UavState& uav_state, const mrs_msgs::HwApiAttitudeCmd& reference,
  ...
  if (parasitic_heading_rate_compensation) {
    ...
    try {
      ...
      // TODO: change to old commented code
      // if(_run_type_!="uav"){
